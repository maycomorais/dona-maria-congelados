...
const _acompsDisponiveisEdit = () =>
  acompanhamentos.filter(a => a.ativo !== false && a.nome !== 'Proteína' && a.nome !== 'Seleta')
    .map(a => ({ nome: a.nome, base: a.gramas_padrao }))
  .length
    ? acompanhamentos.filter(a => a.ativo !== false && a.nome !== 'Proteína' && a.nome !== 'Seleta').map(a => ({ nome: a.nome, base: a.gramas_padrao }))
    : [{ nome:'Arroz Branco',base:80 },{ nome:'Feijão Preto',base:90 },{ nome:'Purê de Batata',base:120 },{ nome:'Purê de Batata Doce',base:120 },{ nome:'Purê de Abóbora',base:120 },{ nome:'Macarrão',base:120 }];
...
function renderizarPratosEdicao() {
  ...
  if (!lista) return;
  const disponiveis = _acompsDisponiveisEdit();
    ...
    return `<div style="background:#fff;border:1.5px solid #e5e4e0;border-radius:9px;margin-bottom:8px;overflow:hidden">
      <div style="display:flex;align-items:center;gap:10px;padding:9px 12px">
        <div style="flex:1;min-width:0">
          ...
          ${pr.linhaNome||pr.linha?`<span style="font-size:.72rem;color:#6b7280;background:#f3f4f6;border-radius:3px;padding:1px 6px;margin-left:5px">${pr.linhaNome||pr.linha}</span>`:''}
          ${tags ? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:5px">${tags}</div>` : ''}
        </div>
        <div style="display:flex;align-items:center;gap:4px;background:#f5f4f0;border-radius:6px;padding:2px 6px;flex-shrink:0">
          <button onclick="editQtd(${i},-1)" style="width:22px;height:22px;border:none;background:none;cursor:pointer;color:#6b7280">−</button>
          <span style="font-weight:700;min-width:20px;text-align:center;font-size:.9rem">${pr.qtd||1}</span>
          <button onclick="editQtd(${i},1)" style="width:22px;height:22px;border:none;background:none;cursor:pointer;color:#6b7280">+</button>
        </div>
        <button onclick="editToggleAcomps(${i})" style="width:26px;height:26px;border:1px solid #d1fae5;background:#ecfdf5;color:#059669;border-radius:6px;cursor:pointer;font-size:.75rem;flex-shrink:0" title="Acompanhamentos"><i class="fas fa-utensils"></i></button>
...
/* Sincronizar lançamento financeiro — upsert via pedido_id */
async function sincronizarLancamentoPedido(pedido) {
  ...
  // Resiliente: se coluna pedido_id não existe, ignora silenciosamente
  const {error}=await supabaseClient.from('lancamentos').upsert(payload,{onConflict:'pedido_id'});
  if(error && !error.message?.includes('column') && !error.message?.includes('schema cache')) console.warn('[Lancamento]',error.message);
  if(!error){
...
function mostrarAbaCardapio(aba) {
  ...
  const filtros = document.getElementById('filtro-linha-cardapio')?.closest('.filtros-pedidos');
  if (filtros) filtros.style.display = isAcomp ? 'none' : '';
...
/* ══════════════════════════════════════════════════════════════
   ACOMPANHAMENTOS ADMIN
   SQL: CREATE TABLE acompanhamentos (
...
async function carregarAcompanhamentosAdmin() {
  ...
  if (error) {
    if (error.message?.includes('schema cache') || error.message?.includes('relation')) {
      // Tabela não existe ainda — mostra aviso na aba
      acompanhamentos = [];
      const tbody = document.getElementById('tbody-acomp-admin');
      if (tbody) tbody.innerHTML = `<tr><td colspan="5">
        <div style="padding:20px;background:#fef9c3;border:1px solid #fde68a;border-radius:8px;text-align:center">
          <strong style="color:#92400e">⚠️ Tabela não encontrada</strong><br>
          <span style="color:#78350f;font-size:.88rem">Execute a migration SQL no painel do Supabase para criar a tabela <code>acompanhamentos</code></span>
        </div></td></tr>`;
    } else {
      console.warn('Acompanhamentos:', error.message);
    }
    return;
  }
...
function renderizarTabelaAcomps() {
  ...
  if (!acompanhamentos.length) {
    const temTabela = !document.querySelector('#tbody-acomp-admin td[colspan="5"] div');
    if (temTabela) tbody.innerHTML = '<tr><td colspan="5" class="empty-msg">Nenhum acompanhamento cadastrado.<br><button class="btn btn-primary btn-sm" style="margin-top:8px" onclick="abrirModalAcomp()"><i class="fas fa-plus"></i> Criar primeiro</button></td></tr>';
    return;
  }
  tbody.innerHTML = acompanhamentos.map(a => `
    <tr>
      <td>
        ...
        <div style="font-size:.72rem;color:#9ca3af;font-family:monospace">GRAM_RULES['${a.nome}'] = {base:${a.gramas_padrao}, extra:${a.preco_extra_10g}}</div>
      </td>
      <td><span style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:5px;padding:3px 10px;font-weight:600;color:#0369a1">${a.gramas_padrao}g</span></td>
      <td><span style="background:#fefce8;border:1px solid #fde68a;border-radius:5px;padding:3px 10px;font-weight:600;color:#854d0e">₲ ${(a.preco_extra_10g||0).toLocaleString('es-PY')} / 10g</span>${!a.preco_extra_10g?'<span style="font-size:.75rem;color:#9ca3af;margin-left:6px">(sem cobrança extra)</span>':''}</td>
      <td><span class="badge badge-${a.ativo?'green':'gray'}">${a.ativo?'Ativo':'Inativo'}</span></td>
      <td style="display:flex;gap:6px">
        <button class="btn btn-sm btn-outline" onclick="editarAcomp('${a.id}')"><i class="fas fa-edit"></i></button>
        <button class="btn btn-sm" style="background:#e74c3c;color:#fff" onclick="excluirAcomp('${a.id}','${a.nome.replace("'","\'")}')"><i class="fas fa-trash"></i></button>
      </td>
    </tr>`).join('');
}
...
function _atualizarPreviewAcomp() {
  ...
  if (!txt) return;
  const taxaEx = Math.ceil(30 / 10) * extra;
  txt.innerHTML =
    `<code style="background:#dcfce7;padding:2px 6px;border-radius:4px">GRAM_RULES['${nome}'] = { base: ${base}, extra: ${extra} }</code>` +
    (extra > 0
      ? `<br><span style="margin-top:4px;display:inline-block">Exemplo: ${base+30}g (+30g) → <strong>₲ ${taxaEx.toLocaleString('es-PY')}</strong> extra</span>`
      : `<br><span style="margin-top:4px;display:inline-block;color:#6b7280">Sem cobrança extra</span>`);
}
...
async function salvarAcomp() {
  ...
  if (error) {
    if (error.code === '23505') { mostrarToast(`Nome "${nome}" já existe`, 'error'); return; }
    if (error.message?.includes('schema cache') || error.message?.includes('relation')) {
      mostrarToast('Tabela não existe — execute a migration SQL no Supabase', 'error'); return;
    }
